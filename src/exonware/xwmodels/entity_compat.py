#!/usr/bin/env python3
"""
Compatibility facade for xwmodels XWModelEntity.
...
class XWModelEntity(XWEntity):
    """xwmodels-compatible model entity facade."""
    ...
    def _redact_sensitive(value: Any) -> Any:
        if isinstance(value, dict):
            ...
            for key, child in value.items():
                if key.lower() in XWModelEntity._SENSITIVE_FIELDS:
                    redacted[key] = "***REDACTED***"
                else:
                    redacted[key] = XWModelEntity._redact_sensitive(child)
            return redacted
        if isinstance(value, list):
            return [XWModelEntity._redact_sensitive(item) for item in value]
        return value
...
# Backward-compatibility alias: keep old xwmodels name working.
XWEntity = XWModelEntity
