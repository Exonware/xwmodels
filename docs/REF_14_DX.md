...
```python
from exonware.xwmodels import XWModelCollection, XWEntity

# XWModelCollection[TEntity: XWEntity] — add/find return typed entities
coll: XWModelCollection[XWEntity] = XWModelCollection("users", entity_type="user")
coll.add(entity)  # entity must be XWEntity
found = coll.find(lambda e: e.get("status") == "active")  # list[XWEntity]
```
...
- `from exonware.xwmodels import XWGroup, XWModelCollection, XWEntity, ...` (primary)
