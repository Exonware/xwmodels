# XWModelEntity New Features - Implementation Summary
...
In xwmodels, the model-facing class name is `XWModelEntity` (with `XWEntity` kept as a compatibility alias).
...
```python
from exonware.xwmodels import XWModelEntity
from exonware.xwschema import XWSchema
from exonware.xwaction import XWAction, ActionProfile

class UserEntity(XWModelEntity):
