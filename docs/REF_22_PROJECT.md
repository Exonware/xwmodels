...
xwmodels provides **entity models** (BaaS-style): entities, collections, groups, auth, cache, lifecycle, relationships, storage integration. It **extends xwentity** with generic types (XWModelCollection[TEntity: XWEntity]) and higher-level document/collection models for Firebase Firestore parity.
...
2. **xwentity injection:** Entity model = XWEntity; collection = XWModelCollection; group = XWGroup. xwentity is finished; xwmodels uses it.
...
| FR-002 | Generic types (XWModelCollection[TEntity: XWEntity]) | High | Done |
...
- **Current phase:** Alpha (Medium). xwentity is finished; xwmodels injects XWEntity into entity model, XWModelCollection into collection model, XWGroup into group model. Generic types documented.
