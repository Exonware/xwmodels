...
1. **Entity model = XWEntity** — xwmodels re-exports XWEntity from `exonware.xwentity`; entity model uses it directly. No re-implementation.

2. **Generic collection** — `XWModelCollection[TEntity: XWEntity]` extends `ACollection[TEntity]`; bounds on `XWEntity` ensure type-safe add/find/remove.

3. **Model group = XWGroup** — `XWGroup` (alias for `XWModelGroup`) extends `AGroup`; `create_collection` returns `XWModelCollection`.

4. **Mapping:**
   - Entity model → XWEntity (from xwentity)
   - Collection model → XWModelCollection[TEntity: XWEntity]
   - Group model → XWGroup (XWModelGroup)

**Why generic types:** Type-safe collections; IDE autocomplete; refactoring safety.

**Implemented:** 08-Mar-2026. `collection.py` defines `XWModelCollection[TEntity: XWEntity]`; `group.py` defines `XWModelGroup` with `XWGroup = XWModelGroup` alias.
