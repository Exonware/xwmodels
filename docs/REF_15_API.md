...
- **Provides:** Entity models (BaaS-style): XWGroup, XWModelCollection[TEntity: XWEntity], XWEntity (from xwentity). Storage/auth provider contracts.
...
```python
from exonware.xwmodels import XWGroup, XWModelCollection, XWEntity
from pathlib import Path

group = XWGroup("my_group", base_path=Path("./data"))
coll = group.create_collection("users", entity_type="user")
entity = XWEntity(schema={...}, data={"name": "Alice"})
coll.add(entity)
group.save_all()
```
...
### XWModelCollection[TEntity: XWEntity]
...
- **Generic:** `TEntity` must be `XWEntity` or subclass; type-safe add/find.

### XWEntity

- **Purpose:** From xwentity (re-exported). Schema + actions + data.
- **See:** [xwentity REF_15_API](../../xwentity/docs/REF_15_API.md)
...
- [xwentity REF_15_API](../../xwentity/docs/REF_15_API.md) — XWEntity API
