...
|---------------|---------------|-------|-------|
| **Entity model** | `XWEntity` | `exonware.xwentity` | Re-exported; schema + actions + data |
| **Collection model** | `XWModelCollection[TEntity: XWEntity]` | `collection.py` | Generic over entity type; extends `ACollection[TEntity]` |
...
+-- collection.py     # XWModelCollection[TEntity: XWEntity] (ACollection[TEntity]); add, find, save
...
**Entry points:** `exonware.xwmodels` (facade, XWGroup, XWModelCollection, XWEntity re-export from xwentity).
...
**Key types:** `XWModelCollection[TEntity: XWEntity]` (ACollection[TEntity], XWObject); add(entity: TEntity), find, save.

### Entity

**Purpose:** From **xwentity** (re-exported). Entity model = XWEntity. Schema + list of actions + data; actions per entity.
...
- **xwentity** — Unified entity (XWEntity); xwmodels extends it, does not re-implement.
